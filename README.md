...
*   **Diseñador de Entidades Dinámico (Entity Manager):** CRUD web para crear tablas físicas en SQL Server y sus respectivas propiedades metadata al instante.
*   **Distribución de UI Inteligente:** Sistema nativo de renderizado asimétrico estandarizado en una cuadrícula equilibrada de 2 columnas de ancho de Bootstrap 5.
*   **Vistas Maestro-Detalle Dinámicas:** Auto-descubrimiento de relaciones e integración automática de sub-tarjetas apiladas verticalmente para módulos vinculados.
*   **Mecanismo de Lookups y Autocompletado:** Buscador relacional de llaves foráneas con soporte para modales genéricos y sugerencias fluidas en tiempo real.
...
### 2. Base de Datos
Ejecuta los scripts ubicados en la carpeta `/Database` (incluyendo los stored procedures del núcleo como `sp_Core_CreateEntity`, `sp_Core_AddProperty`, `sp_Core_DropProperty` y `sp_Core_UpdatePropertyMetadata`) en tu instancia de SQL Server.

### 3. Configuración de Credenciales
Actualiza la cadena de conexión y los parámetros del administrador en el archivo `OpenPlaDiC.WebApp/appsettings.json`:
